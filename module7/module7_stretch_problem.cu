...
__host__ void generate_rand_data(unsigned int * data, unsigned int num_elements)
{
	...
	{
		// generates random int between 0-32767
		data[i] = rand();
	}
}
...
__host__ float execute_register_memory_operations(void)
{
	...
	//free device and host memory allocations (host memory allocated on stack, so it will be freed automatically)
	cudaFree(d_data);
...
__host__ float execute_shared_memory_operations()
{
	...
	// host memory was allocated on the stack, not allocated using cudaMallocHost, so no need to free
